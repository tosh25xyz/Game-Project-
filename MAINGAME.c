...
int main()
{
    ...
    // Force the window onto the primary monitor and un-minimize it.
    // Without this some driver/monitor setups start the window hidden
    // or minimized in the taskbar (typically at the -32000 off-screen spot).
    ...
    while (!WindowShouldClose())
    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (playerstats.currentHp <= 0) playerstats.currentHp = 1;
                        mode = MODE_OVERWORLD;
                    }
                }
            }
        ...
        else if (mode == MODE_OVERWORLD || mode == MODE_CAVE)
        {
            ...
            // "Your Health Has Been Restored" banner after winning a battle
            if (restoreTimer > 0.0f)
            {
                restoreTimer -= dt;
                const char *restoreMsg = "Your Health Has Been Restored";
                int msgWidth = MeasureText(restoreMsg, 30);
                DrawText(restoreMsg, (screenWidth - msgWidth) / 2, 180, 30, GREEN);
            }
        }
