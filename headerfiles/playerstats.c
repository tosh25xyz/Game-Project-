...
void GainExp(PlayerStats *stats,int amount)
{
    ...
    while(stats->currentExp>=stats->expToNextLevel)
    {
        ...
        stats->expToNextLevel=stats->level*BASE_EXP_TO_LEVEL;
    }
}
void DrawPlayerHud(PlayerStats *stats,int x,int y)
{
    ...
    DrawText(namelevel,x,y,40,WHITE);
    ...
    DrawRectangle(x,y+24,BarWidth,BarHeight,DARKGRAY);
    Color hpColor=GREEN;
    if(hpPct<0.5f )
    {
        hpColor=YELLOW;
    ...
    char exptext[32];
    snprintf(exptext,sizeof(exptext),"EXP %d/%d",stats->currentHp,stats->expToNextLevel);
    DrawText(exptext,x,y+58,12,LIGHTGRAY);
